...
with psycopg2.connect(dbname="chembl_29", user="user", password="user", host="192.168.54.19") as conn:
    with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cursor:
        with open("/home/chengyj/kinase_work/dataset/Bridged_ring/chembl_rings/KI_chembl/bridged_ki.csv") as infor:
            for line in infor:
                ...
                for item in results:
                    ...
                    if 'kinase' in pref_name:
                        ...
                        if bio and assay_type == "IC50":
                            out_result.append((float(bio),pref_name,assay_type))
                        else:
                            pass
                if len(out_result) > 2:
                    out_result = sorted(out_result, key = lambda x:x[0] )
                    print(f"{molrego:>8} {out_result[-2][0]/out_result[0][0]:>.2f} |{out_result[0][1]}| {out_result[-1][1]} IC50")

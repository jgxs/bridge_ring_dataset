...
ligand_r6 = []
with open(
    "/home/chengyj/kinase_work/dataset/Bridged_ring/PDB_rings/lig_in_pdb/pdb_dataset/Components-smiles-stereo-oe.smi"
) as inputfile:
    for line in tqdm(inputfile):
        try:
            ...
            if (
                ...
                and not mol.GetSubstructMatches(patt_not1)
                and not mol.GetSubstructMatches(patt_not2)
                and not mol.GetSubstructMatches(patt_not3)
                and not mol.GetSubstructMatches(patt_not4)
            ):
